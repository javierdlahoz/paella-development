...
if(is_admin()){
	...
	*/
	...
	function wpv_filter_post_relationship_delete_callback() {
		...
		if ( isset( $view_array['post_relationship_shortcode_attribute'] ) ) {
			unset( $view_array['post_relationship_shortcode_attribute'] );
		}
		if ( isset( $view_array['post_relationship_url_parameter'] ) ) {
			unset( $view_array['post_relationship_url_parameter'] );
		}
...
function wpv_get_filter_post_relationship_summary_txt($view_settings, $short=false) {
	...
	if ($view_settings['post_relationship_mode'] == 'current_page') {
		...
		echo sprintf( __('Select related posts that are a <strong>child</strong> to the <strong>Post with ID set by the shortcode attribute %s</strong>.', 'wpv-views'), $view_settings['post_relationship_shortcode_attribute'] );
		echo sprintf( __(' eg. [wpv-view name="view-name" <strong>%s</strong>="123"]', 'wpv-views'), $view_settings['post_relationship_shortcode_attribute'] );
	} else if ($view_settings['post_relationship_mode'] == 'url_parameter') {
		echo sprintf( __('Select related posts that are a <strong>child</strong> to the <strong>Post with ID set by the URL parameter %s</strong>.', 'wpv-views'), $view_settings['post_relationship_url_parameter'] );
		echo sprintf( __(' eg. http://www.example.com/my-page/?<strong>%s</strong>=123', 'wpv-views'), $view_settings['post_relationship_url_parameter'] );
	} else {
