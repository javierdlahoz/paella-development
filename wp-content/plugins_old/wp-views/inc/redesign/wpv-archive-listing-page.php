		...
		while ($wpv_query->have_posts()) :
			...
			<tr id="wpv_view_list_row_<?php echo $post->ID; ?>" class="js-wpv-view-list-row<?php echo $alternate; ?>" >
				...
				<td  class="wpv-admin-listing-col-usage">
					...
					<?php
				//	$opt2 = $WPV_view_archive_loop->_view_edit_options($post->ID, $options); TODO where the hell is this used
					$selected = '';
    ...
    ?>
        <table id="wpv_view_list_usage" class="js-wpv-views-listing wpv-views-listing wpv-views-listing-by-usage widefat">
			...
			$alternate = '';
                    ...
                    <?php if (is_null($post)): ?>
                        <td colspan="2">
                            <a class="button js-create-view-for-archive" data-forwhom="<?php echo $loop_name; ?>" href="#"><i class="icon-plus"></i><?php _e('Create a WordPres Archive for this loop');?></a>
                        </td>
                    <?php else: ?>
                    <td class="wpv-admin-listing-col-title">
