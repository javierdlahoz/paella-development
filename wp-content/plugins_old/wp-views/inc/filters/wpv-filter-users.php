...
if(is_admin()){
	...
	*/
	...
	function wpv_get_list_item_ui_users($selected, $view_settings = null) {
		...
		$td .= "</p>\n<p class='edit-filter js-wpv-filter-edit-controls'>\n<button class='button-secondary js-wpv-filter-edit-open js-wpv-filter-users-edit-open'>".
		__('Edit','wpv-views') ."</button>\n<i class='icon-remove-sign js-filter-remove' data-nonce='". wp_create_nonce( 'wpv_view_filter_users_delete_nonce' )
		. "'></i>\n</p>";
...
function wpv_get_filter_users_summary_txt($view_settings, $short=false, $post_id='') {
	...
	case 'by_url':
		...
		if ('' != $example) echo sprintf(__(' eg. yoursite/page-with-this-view/?<strong>%s</strong>=%s', 'wpv-views'), $url_users, $example);
		break;
	case 'shortcode':
