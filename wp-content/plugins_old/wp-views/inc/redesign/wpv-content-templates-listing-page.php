			...
			$count = sizeof( $posts );
				if ( $count > 0 ) {
					...
					if ( ( $count - $set_count ) > 0){
					  ...
					  data-target="'.admin_url('admin-ajax.php').'?action=wpv_ct_update_posts&type='.$type.'&tid='.$ct_id.'&wpnonce='.wp_create_nonce( 'work_view_template' ).'"></a>';
					}
				}
			$list .= '</li>';
		}
					...
					<ul>
						...
						if ( isset( $options['views_template_for_' . $type] ) ) {
							if ( $options['views_template_for_' . $type] != 0 ) {
								...
								if ( is_object( $template ) ){
									...
									if ( $count > 0 ) {
										...
										if ( ( $count - $set_count ) > 0){
											...
											data-target="'.admin_url('admin-ajax.php').'?action=wpv_ct_update_posts&type='.$type.'&tid='.$template->ID.'&wpnonce='.wp_create_nonce( 'work_view_template' ).'"></a> '. ( $count - $set_count ) .' '. $label .' use a different template '.
								...
								echo $add_button;
								if ( $set_count > 0) { ?>
									<a class="button js-single-unlink-template-open-dialog" href="#" data-unclear="<?php echo $set_count; ?>" data-slug="<?php echo $type; ?>" data-label="<?php echo htmlentities( $label, ENT_QUOTES ); ?>"><i class="icon-unlink"></i>
