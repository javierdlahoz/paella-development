...
function wpv_update_title_description_callback() {
	...
	$title_check = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_title = '" . mysql_real_escape_string($_POST["title"]) . "' AND post_type='view' AND ID!=". $return ." " );
	...
	$name_check = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_name = '" . mysql_real_escape_string($_POST["slug"]) . "' AND post_type='view' AND ID!=". $return ." " );
	if ( !empty($name_check)  ){
...
function wpv_create_view_callback() {
	...
	if ( 0 != $id ) {
		if (!isset($_POST["kind"])) $_POST["kind"] = 'normal';  // TODO check if kind is needed anymore, see below
		if (!isset($_POST["purpose"])) $_POST["purpose"] = 'full';
		switch($_POST["kind"]) {
		case 'archive': // TODO maybe not needed anymore, we have a specific callback for this
		...
		default:
			...
			if ( $_POST["purpose"] == 'slider' ){ // if the purpose is slider, create a CT for every slide, add it to the View layout and to the list of connected CT
...
function wpv_scan_view_callback() {
    ...
    $needle = '%[wpv-view%name="%' . mysql_real_escape_string($view->post_title). '%"]%';
    $needle = mysql_real_escape_string( $needle );
    $needle_name = '%[wpv-view%name="%' . mysql_real_escape_string($view->post_name). '%"]%';
    $needle_name = mysql_real_escape_string( $needle_name );
